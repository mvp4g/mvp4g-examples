...
public final class ContentView
  ...
  private static final PersonProperties personProperties = GWT.create(PersonProperties.class);
  ...
  private ListStore<Person> storePerson;
...
//    gridPerson.setWidth("100%");
  ...
  private void bind() {
    ...
    tabPanel.addCloseHandler(new CloseEvent.CloseHandler<Widget>() {
      ...
      public void onClose(CloseEvent<Widget> event) {
        tabItemValue = tabs.get(event.getItem()
                                     ...
                                     .getId());
        if (tabItemValue != null) { // TabItem gefunden?
          getPresenter().doClose(event.getItem()
                                      .getElement()
                                      .getId());
        }
      }
    ...
    // Grid Handler
    gridPerson.addRowDoubleClickHandler(new RowDoubleClickHandler() {
      @Override
      public void onRowDoubleClick(RowDoubleClickEvent rowDoubleClickEvent) {
        Person person = storePerson.get(rowDoubleClickEvent.getRowIndex());
        if (person != null) {
          getPresenter().doShowPerson(person.getId());
        }
      }
    });
  }
  ...
  public void selectUebersicht() {
    ...
    getPresenter().doUpdatePersonGrid();
  }

  @Override
  public void edit(List<Person> people) {
    storePerson.clear();
    storePerson.addAll(people);
  }
