...
import org.gwt4e.mvp4g.example.modules.client.company.view.CompanyListView;
...
public class CompanyListPresenter extends LazyPresenter<CompanyListPresenter.CompanyListViewInterface, CompanyEventBus> {
	...
	public void bindView() {
		...
		view.isDisabledApplicationHistory()
				.addValueChangeHandler(new ValueChangeHandler<Boolean>() {

					@Override
					public void onValueChange(ValueChangeEvent<Boolean> event) {
						eventBus.setApplicationHistoryStored(!event.getValue()
																											 .booleanValue());
					}

				});
		view.isDisabledModuleHistory()
				.addValueChangeHandler(new ValueChangeHandler<Boolean>() {
